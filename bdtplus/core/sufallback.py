...
from bdtplus.core.superfastpath_cp import sufastpath
from bdtplus.core.hsfastpath import hsfastpath
...
BroadcastReceiverQueues = namedtuple(
    'BroadcastReceiverQueues', ('FAST', 'BOLT', 'VIEW_CHANGE'))
...
class SUFB():
    ...
    def run_bft(self):
        ...
        muted_nodes = [node for node in range(1, int((self.N - 1) / 3) + 1)]
        ...
        while True:
            ...
            def make_epoch_send(e):
                def _send(j, o):
                    if time.time()-self.s_time<15 or self.id not in muted_nodes:
                        self._send(j, (e, o))
                return _send
    ...
    def _run_epoch(self, e, send, recv):
        ...
        fixed_block = None
        latest_sudelivered_block = None
        latest_sudeliver_data = None
        ...
        def _setup_sufastpath(leader):

            def sufastpath_send(k, o):
                send(k, ('FAST', o))

            def fastpath_output(o):
                nonlocal latest_sudelivered_block, latest_sudeliver_data
                ...
                latest_sudelivered_block, latest_sudeliver_data = o
                fast_blocks.put(o)

            sufast_thread = gevent.spawn(sufastpath, epoch_id, pid, N, f, leader,
                                   self.transaction_buffer.get_nowait, fastpath_output,
                                   self.SLOTS_NUM, self.FAST_BATCH_SIZE, T,
                                   hash_genesis, self.sPK2s, self.sSK2,
                                   fast_recv.get, sufastpath_send, logger=self.logger, omitfast=self.omitfast)

            return sufast_thread
        ...
        def _setup_fastpath(leader):
            ...
            fast_thread = gevent.spawn(hsfastpath, epoch_id, pid, N, f, leader, su_vote, su_block, st,
                                   ...
                                   bolt_recv.get, fastpath_send, logger=self.logger, omitfast=self.omitfast)

            return fast_thread
        # Start the fast path

        sufast_thread = _setup_sufastpath(leader)

        # Wait either view_change handler done or fast_path done
        vc1_ready = gevent.event.Event()
        vc1_ready.clear()
        ...
        gevent.spawn(wait_for_sufastpath)
