...
from bdtplus.core.sufallback import SUFB
...
class SUFBNode (SUFB):
    ...
    def __init__(self, sid, id, S, T, Bfast, Bacs, N, f, l, bft_from_server: Callable, bft_to_client: Callable, ready: mpValue, stop: mpValue, K=3, mode='debug', mute=False, bft_running: mpValue=mpValue(c_bool, False), omitfast=False):
        ...
        SUFB.__init__(self, sid, id, S, T, max(int(Bfast), 1), max(int(Bacs/N), 1), N, f, l, self.sPK, self.sSK, self.sPK1, self.sSK1, self.sPK2s, self.sSK2, self.ePK, self.eSK, send=None, recv=None, K=K, mute=mute, omitfast=omitfast)
        ...
        if self.mode == 'test' or 'debug': #K * max(Bfast * S, Bacs)
            for _ in range(self.K + 1):
                for r in range(self.SLOTS_NUM):
                    ...
                    SUFB.submit_tx(self, tx[:-len(suffix)] + suffix)
                    if r % 50000 == 0:
...
def main(sid, i, S, T, B, N, f, addresses, K):
    mule = SUFBNode(sid, i, S, T, B, N, f, addresses, K)
    mule.run_bft()
